...
public class TwilioManagerImpl extends AbstractMirrorManager implements TwilioManager {
// ------------------------------ FIELDS ------------------------------
...
// --------------------- Interface MirrorManager ---------------------
    ...
    public void onFeatureResume() {
        ...
        // need all the properties to be present
        if (isEmpty(mProfileSID) || isEmpty(mAccountSID) || isEmpty(mApiKeySID) || isEmpty(mApiKeySecret) ||
                isEmpty(mIdentity)) {
            disable();
            return;
        }

        // Twilio has already been initialized
...
// --------------------- Interface TwilioManager ---------------------
    ...
    private AccessManager.Listener accessManagerListener() {
        ...
        return new AccessManager.Listener() {
            ...
            public void onTokenExpired(AccessManager twilioAccessManager) {
                Timber.i("onTokenExpired(AccessManager)");
                authorizeClient();
            }

            @Override
            public void onTokenUpdated(AccessManager twilioAccessManager) {
                Timber.i("onTokenUpdated(AccessManager)");
                mConfigurationManager.enablePresenter(TwilioPresenter.class);
                startConversationsClient();
            }

            @Override
            public void onError(AccessManager twilioAccessManager, String s) {
                Timber.e("onError(AccessManager,String) %s", s);
                disable();
            }
        };
    }
    ...
    private void authorizeClient() {
        ...
        mAccessManager =  new AccessManager(mAppManager.getAppContext(), token.toJWT(), accessManagerListener());
    }
    ...
    private void disable() {
        mConfigurationManager.disablePresenter(TwilioPresenter.class);
        mProfileSID = null;
        mAccountSID = null;
        mApiKeySID = null;
        mApiKeySecret = null;
