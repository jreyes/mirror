...
// ------------------------------ FIELDS ------------------------------
    ...
    private final AuthorizationDialogController mUIController;

// --------------------------- CONSTRUCTORS ---------------------------

    public OAuthManager(AuthorizationFlow flow, AuthorizationDialogController uiController) {
        this.mFlow = flow;
...
// -------------------------- OTHER METHODS --------------------------
    ...
    private void submitTaskToExecutor(final Future2Task<?> task) {
        // run the task in a background thread
        mExecutor.submit((Runnable) task::start);
    }
...
// -------------------------- INNER CLASSES --------------------------
    ...
    public abstract class Future2Task<T> extends BaseFutureTask<T> implements
        ...
        protected void done() {
            if (mCallback != null) {
                postRunnableToHandler(() -> mCallback.run(Future2Task.this));
            }
        }
        ...
        private void ensureNotOnMainThread() {
            ...
            if (looper != null && looper == Looper.getMainLooper()) {
                ...
                Timber.w(exception, "calling this from your main thread can lead to deadlock and/or ANRs");
                throw exception;
            }
        }
    }
    ...
    private abstract class BaseFutureTask<T> extends FutureTask<T> {
        ...
        BaseFutureTask(Handler handler) {
            super(() -> {
